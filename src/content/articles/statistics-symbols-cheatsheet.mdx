...
The denominator. σ describes a whole population and divides by N; s is computed from a sample and divides by n − 1 (Bessel's correction, which removes a systematic bias). In practice you almost always use s, because you rarely have data for an entire population. As n grows large the two converge: for the eight values in the worked example σ ≈ 5.339 and s ≈ 5.707, but at n = 1 000 the gap is under 0.1 %.
...
Within probability, the two essential set symbols are **∩** ("and," intersection) and **∪** ("or," union) — use the addition rule P(A ∪ B) = P(A) + P(B) − P(A ∩ B) whenever you combine events.
