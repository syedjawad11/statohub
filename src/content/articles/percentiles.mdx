...
Four steps. (1) Sort the data in ascending order. (2) Compute position = (p / 100) x (n - 1). (3) Set j = floor(position), k = ceil(position), f = position - j. (4) If f = 0, the answer is sortedData[j]; otherwise the answer is sortedData[j] + f x (sortedData[k] - sortedData[j]). Apply this procedure for any p from 0 to 100.
