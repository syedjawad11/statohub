...
Statohub's <Link to={routes.categoryHub('data-analysis')}>Data Analysis</Link> guides cover the exploratory groundwork this kind of project needs before any regression gets fit, and a general-purpose <Link to={routes.calculator('average')}>average calculator</Link> can help you sanity-check summary figures like the pre-intervention monthly mean before you commit to a full model.
