...
Only if at least one event is impossible (has probability zero). For any two events with P(A) > 0 and P(B) > 0, mutual exclusivity requires P(A ∩ B) = 0, while independence requires P(A ∩ B) = P(A) × P(B) > 0. Both conditions cannot hold simultaneously unless one probability is zero — which means the event never occurs. In any real probability problem with actual events, the answer is: **no**, two non-trivial events cannot be both independent and mutually exclusive. The [NIST/SEMATECH e-Handbook of Statistical Methods, Section on Probability](https://www.itl.nist.gov/div898/handbook/eda/section3/eda362.htm) states the same result.
