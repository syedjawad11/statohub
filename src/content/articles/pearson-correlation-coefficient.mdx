...
The strength and direction of the linear relationship between two continuous numerical variables — in other words, how closely the paired values follow a straight-line pattern. The result ranges from −1 (perfect negative linear relationship) to +1 (perfect positive linear relationship), with 0 indicating no linear association.

### How is Pearson's r related to covariance?

Cov(X, Y) is the average cross-product of deviations from the two means, and Pearson's r is that covariance divided by the product of the two standard deviations: r = Cov(X, Y) / (sX × sY). This division standardises the covariance, removing the effect of the variables' units and scaling the result to the [−1, +1] range.
