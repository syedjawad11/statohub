...
Pearson correlation assumes both variables are continuous and approximately normally distributed, and that the relationship between them is linear rather than curved. When these assumptions are violated — for example, with ordinal variables or a clearly non-linear relationship — use Spearman's rank correlation instead. The <Link to={routes.article('correlation-coefficient')}>Pearson correlation coefficient guide</Link> explains both versions and their assumptions.
