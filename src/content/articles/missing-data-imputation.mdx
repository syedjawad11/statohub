...
If you're working through a real project right now, start with the Applied Statistics hub for walkthroughs that connect theory to messy, real datasets, then use the <Link to={routes.calculator('average')}>Average Calculator</Link> or <Link to={routes.calculator('chi-square')}>Chi Square Calculator</Link> to double-check your pooled estimates by hand before you commit to a final number.
