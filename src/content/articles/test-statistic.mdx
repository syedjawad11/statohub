...
One independent piece of information that is free to vary when estimating a parameter. In a sample of n values, all n observations can vary freely. Once you use that sample to estimate the mean, that constraint "uses up" one degree of freedom, leaving n − 1 degrees of freedom for estimating the variance. The [Penn State STAT 415 course on hypothesis testing for one mean](https://online.stat.psu.edu/stat415/lesson/7/7.1) discusses degrees of freedom in detail and shows how they shape the t-distribution.
