...
## How to Average Multiple r Values
...
The average of several Pearson r values from different studies or subgroups. Because the r scale is compressed near ±1, raw r values cannot simply be averaged. Convert each r to Fisher's z' = 0.5 × ln[(1 + r)/(1 − r)], average the z' values (weighting by nᵢ − 3 if sample sizes differ), then back-transform with r = tanh(z'_mean), as in the worked example above.
