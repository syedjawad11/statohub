...
Correlation appears in almost every branch of statistics — from medical research to finance to education — because it answers a straightforward question: do two measured quantities tend to move together? This article builds the correlation definition up from first principles, then covers positive, negative and zero correlation, what the number does and does not tell you, how to average several r values, and how to read a correlation table. The formula itself, a step-by-step calculation and the ways r can mislead are the job of the <Link to={routes.article('correlation-coefficient')}>correlation coefficient guide</Link>; this page is about the concept.
...
Pearson's r is built from how each pair of observations deviates from its own mean: when x sits above its mean at the same time y sits above its mean (or both sit below), the pair pushes r toward +1; when the two deviate in opposite directions, the pair pushes r toward −1. Dividing by the spread of each variable keeps the result between −1 and +1 and strips out the units, so r is a pure number that can be compared across variables measured on completely different scales. The [NIST/SEMATECH e-Handbook of Statistical Methods, section on the Linear Correlation Coefficient](https://www.itl.nist.gov/div898/handbook/prc/section2/prc24.htm) gives the derivation; the <Link to={routes.article('correlation-coefficient')}>correlation coefficient guide</Link> works through the formula and a full hand calculation.
...
Every student who studied more scored higher, and the increases are nearly proportional — each extra hour adds roughly five to ten points. Running the five pairs through the formula gives **r ≈ 0.991**, very close to +1: as study hours increase, exam scores rise in an almost perfectly linear pattern. To compute r for your own data, the <Link to={routes.calculator('correlation-coefficient')}>correlation coefficient calculator</Link> returns r, r², and the regression equation automatically.
...
What a given r tells you depends on both its sign and its magnitude:
...
## How to Average Several r Values
...
The average correlation is approximately **r = 0.68**. Averaging the raw values directly gives (0.50 + 0.80) / 2 = 0.65 — a slight underestimate, because the r scale compresses near ±1.
...
This gives larger samples proportionally more influence, producing a more precise estimate of the average correlation.

## What Correlation Is and Is Not

Using r well means grasping both its power and its limits:
...
Pearson's r covers linear relationships between two continuous variables. Other coefficients handle different data types:
...
Only the direction. With r between 0 and +1 both variables increase together or both decrease together; with r between −1 and 0 one increases as the other decreases. A correlation of +0.80 is exactly as strong as −0.80. Zero correlation means no systematic linear relationship exists.

### Which table shows a positive correlation?

Look at the off-diagonal cells: a matrix whose off-diagonal entries are positive numbers clearly above zero (say, 0.72) is describing variables that rise and fall together. Cells near 0 show little or no linear relationship, and cells near −1 show a strong negative one. In a multiple-choice problem, the table with clearly positive off-diagonal entries is the one to pick.

### Can you average several correlation coefficients?

Not directly. Because the r scale is compressed near ±1, raw r values from different studies or subgroups cannot simply be added and divided. Convert each r to Fisher's z' = 0.5 × ln[(1 + r)/(1 − r)], average the z' values (weighting by nᵢ − 3 if sample sizes differ), then back-transform with r = tanh(z'_mean), as in the worked example above.

---

## Summary

Correlation is a foundational concept in statistics. The **correlation definition** — a measure of the strength and direction of the linear relationship between two variables — underlies regression analysis, factor analysis, and countless research designs. A **positive correlation** (r > 0) means variables rise and fall together; a negative correlation (r < 0) means one rises as the other falls. For combining r values across studies, Fisher's z-transformation is the statistically correct approach. And when reading a correlation matrix, a positive relationship shows up as off-diagonal values that are clearly positive and above the noise threshold of roughly 0.3.
