...
When each subject is measured twice, or subjects are matched across two conditions, the paired t-test converts each pair into a single difference score, then tests whether the mean of those differences is significantly different from zero using the standard student t-test formula applied to the differences. It is the appropriate choice whenever participants are measured twice or matched across two conditions.

### What is the t-test equation?

In every version, t is the observed effect divided by its standard error:
...
When two entirely independent groups are being compared and no natural pairing or matching connects observations across the groups. If the same subjects appear in both conditions, or if observations were deliberately matched before the study, the paired t-test will be more powerful and more appropriate.
...
The t-test family provides a principled framework for comparing means when the population standard deviation is unknown. The **paired t-test** — or paired difference t-test — is the most powerful choice when each subject provides two related observations, because converting the data to difference scores removes between-subject noise from the analysis. The **two-sample t-test** handles independent groups by pooling the two variance estimates into a shared standard error. Both rest on the same logic: the size of the effect relative to how much that effect would vary by chance. Together they cover the vast majority of mean-comparison problems encountered in research, quality control, and data analysis.

Mastering the t-test means knowing when to pair, how to compute difference scores, how to read the t-distribution critical values, and how to interpret p-values in context — not in isolation.
