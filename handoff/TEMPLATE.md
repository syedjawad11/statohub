Status: TODO
<!-- TODO | IN_PROGRESS | DONE | CHANGES_REQUESTED | CLOSED -->

# TASK-NNN -- <short title>

- **Owner of this stage:** <Codex | Claude>
- **Repo:** `Desktop/statohub/` (where the work happens)
- **Created:** <YYYY-MM-DD> by Claude

<!-- Authoring note (Claude): keep this file plain ASCII. Use `--` not an em
dash, `->` not an arrow, `...` not an ellipsis. Codex reads these files through
a Windows codepage; non-ASCII punctuation renders as mojibake and breaks its
apply_patch matching. -->

---

## Brief  *(Claude writes -- what Codex needs to execute)*

**Goal:** <one or two sentences -- the outcome, not the steps.>

**Context / inputs:**
- <links to BUILD-PLAN.md sections, spec, or files Codex should build from>

**Deliverables:**
- [ ] <concrete file or result 1>
- [ ] <concrete file or result 2>

**Constraints:**
- Stay in this repo; don't touch sibling folders.
- <stack / lean / reproducibility constraints specific to this task>

**Definition of done / how to verify:**
- <what "finished and correct" looks like -- a command to run, a page that
  loads, a test that passes>

---

## Work Log  *(Codex writes -- what was actually done)*

- **Started:** <YYYY-MM-DD>
- **Finished:** <YYYY-MM-DD>

**What changed (files + where):**
- <path> -- <what + why>

**How to verify:**
- <exact command / steps you ran and the result>

**Blocked / couldn't do / decisions made:**
- <anything Claude should know -- or "none">

---

## Review  *(Claude writes -- accept or send back)*

- **Reviewed:** <YYYY-MM-DD>
- **Verdict:** <CLOSED | CHANGES_REQUESTED>

**Notes / what to improve:**
- <specifics if sending back; or what was good if closing>
