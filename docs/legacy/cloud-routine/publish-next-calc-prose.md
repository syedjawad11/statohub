...
`content-ops/cloud-routine/PAUSED` exists in the checkout, all statohub cloud
routines are paused by explicit human decision -- print
`PUBLISH_RESULT: paused` (with the file's contents for context) and exit 0
immediately. Do NOT pick a queue item, write, build, or push. This file is the
source of truth for pause state, not this doc's prose. The same marker gates
`publish-next-article.md`; one file pauses both routines.
